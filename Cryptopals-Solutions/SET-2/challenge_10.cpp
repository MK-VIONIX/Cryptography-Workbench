    ...
    while(bytesGenerated < 176){
        ...
        for(int i=0;i<4;i++){
            ...
            roundKey[bytesGenerated] =
                roundKey[bytesGenerated - 16] ^ temp[i];
...
string xor_blocks(string a, string b){
...
int main(){
    ...
    freopen("challenge_10.txt","r",stdin);
    ...
    for(size_t i = 0; i < ciphertext.size(); i += 16){
        ...
        string current_block =
            ciphertext.substr(i, 16);

        string decrypted =
            aes_ecb_decrypt(current_block, key);

        string plain_block =
            xor_blocks(decrypted, previous);
